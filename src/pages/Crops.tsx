...
import {CropSavePopUp} from "../components/CropSavePopUp.tsx";
import {CropUpdatePopUp} from "../components/CropUpdatePopUp.tsx";
import {ViewCropPopUp} from "../components/ViewCropPopUp.tsx";
...
export function Crops() {
    ...
    const handleDelete = (cropId: string) => {
        setCropToDelete(cropId); // Set the staff ID for deletion
        handleDeleteData(cropId , 'Crop'); // Call the delete confirmation popup
    };
    return (
        <>
            <SearchBar
                pagename="Crop Management"
                buttonName="Add Crop"
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mt-40 ml-4 ">
                ...
                <div className="bg-white rounded-lg  overflow-hidden shadow-2xl ">
                    ...
                    <div className="p-4">
                        <p className="text-gray-600 text-sm font-semibold">Crop Code</p>
                        <p className="text-gray-800 font-medium truncate">F-52d02329-a257-4854...</p>
                        <p className="text-gray-600 text-sm font-semibold mt-2">Crop Name</p>
                        <p className="text-gray-800 font-medium">Deas</p>
                        <p className="text-gray-600 text-sm font-semibold mt-2">Crop Season</p>
            ...
            {/*popup add staff*/}
            {PopupSaveVisible && <CropSavePopUp onClose={() => setShowSave(false)}/>}
            {PopUpUpdateVisible && <CropUpdatePopUp onClose={() => setShowUpdate(false)}/>}
            {PopUpViewStaffVisible && <ViewCropPopUp onClose={() => setShowViewStaff(false)}/>}
        </>
    );
}