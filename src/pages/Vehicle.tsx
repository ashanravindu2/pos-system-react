...
import {VehicleSavePopUp} from "../components/VehicleSavePopUp.tsx";
import {VehicleUpdatePopUp} from "../components/VehicleUpdatePopUp.tsx";
import {ViewVehiclePopUp} from "../components/ViewVehiclePopUp.tsx";
...
export function Vehicle() {
    ...
    const [vehicleToDelete, setVhicleToDelete] = useState<string | null>(null); //delete eka mekem ghnnone
    ...
    const handleDelete = (vehicleCode: string) => {
        setVhicleToDelete(vehicleCode); // Set the staff ID for deletion
        handleDeleteData(vehicleCode , 'Vehicle'); // Call the delete confirmation popup
    };
    return (
        <>
            <SearchBar
            ...
            <div className=" flex justify-center mt-52 ml-10 ">
                <div className="">
                    <div className="grid grid-cols-6 gap-4 border-b pb-2 p-2 ">
                        <h5 className="flex flex-col items-center">Vehicle Code</h5>
                        <h5 className="flex flex-col items-center">License Plate No</h5>
                        <h5 className="flex flex-col items-center">Category</h5>
                        <h5 className="flex flex-col items-center">Fuel Type</h5>
                        <h5 className="flex flex-col items-center">Status</h5>
                    ...
                    <div className={'overflow-y-scroll table-col w-auto table-rows' }>
                        <div className="" id="staffTableBody">
                            ...
                            <div className={'grid grid-cols-6 gap-4 mt-3 p-2'}>
                                ...
                                <h5 className="flex flex-col items-center text-emerald-500	 ">1234567890</h5>
            ...
            {/*popup add staff*/}
            {PopupSaveVisible && <VehicleSavePopUp onClose={() => setShowSave(false)} />}
            {PopUpUpdateVisible && <VehicleUpdatePopUp onClose={() => setShowUpdate(false)} />}
            {PopUpViewStaffVisible && <ViewVehiclePopUp onClose={() => setShowViewStaff(false)} />}
